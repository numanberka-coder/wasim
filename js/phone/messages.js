...
function buildMessageRow(msg) {
  ...
  // Grouping classes for margin/spacing
  ...
  // Add grouping modifiers for border-radius
  // Tail is on the FIRST bubble of a group (or standalone)
  ...
  if (msg.replyTo) {
    ...
    if (key) {
      ...
      if (idMatch) {
        ...
        targetMsg = all.find(m => Number(m.id) === idNum) || null;
      }
      ...
      if (!targetMsg) {
        for (let i = all.length - 1; i >= 0; i--) {
          ...
          if (String(m?.text || '').trim() === key) { targetMsg = m; break; }
        }
      }
      ...
      if (!targetMsg) {
        for (let i = all.length - 1; i >= 0; i--) {
          const m = all[i];
          if (m?.id === msg.id) continue;
          if (String(m?.text || '').includes(key)) { targetMsg = m; break; }
        }
      }
      ...
      if (!targetMsg) {
        ...
        for (let i = all.length - 1; i >= 0; i--) {
          const m = all[i];
          if (m?.id === msg.id) continue;
          if (String(m?.speaker || '').toLowerCase() === keyLower) { targetMsg = m; break; }
        }
      }
    }
    ...
    const color = targetMsg?.speaker ? state.getColorForSpeaker(replyName) : '#a78bfa';
  ...
  // Content
  if (msg.kind === 'photo' || msg.kind === 'gif') {
    ...
    const PLAY_ICON  = `<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><polygon points="5 3 19 12 5 21 5 3"/></svg>`;
    ...
    dur.textContent = formatVoiceDuration(msg.durationSec || 12);
    ...
    play.addEventListener('click', (e) => {
      ...
      const totalSec   = Math.max(1, msg.durationSec || 12);
      const bars       = Array.from(wave.querySelectorAll('span'));
      const totalBars  = bars.length;
      const tickMs     = 80; // update every 80ms
      const totalTicks = Math.round((totalSec * 1000) / tickMs);
      let   tick       = 0;

      voice._playTimer = setInterval(() => {
        tick++;
        const progress   = tick / totalTicks;           // 0 → 1
        const currentBar = Math.floor(progress * totalBars);
        const remSec     = Math.max(0, Math.round(totalSec * (1 - progress)));
        dur.textContent  = formatVoiceDuration(remSec);
        ...
        bars.forEach((b, i) => {
          if (i < currentBar) {
            ...
            b.classList.remove('played', 'current');
          }
        });
    ...
    // Location message — map preview card
    const loc = document.createElement('div');
    loc.className = 'msg-location';

    // Map visual: static gradient + pin
    ...
    const extColors = { pdf: '#f44336', doc: '#2196f3', docx: '#2196f3', xls: '#4caf50', xlsx: '#4caf50', ppt: '#ff5722', pptx: '#ff5722', zip: '#9c27b0', rar: '#9c27b0', txt: '#607d8b' };
    const iconColor = extColors[ext] || '#607d8b';
    ...
    // Sticker — transparent, no bubble frame
    ...
    // Link preview — title + thumbnail + URL
    ...
    // Extract domain from url or use durationSec field as site label
    ...
    // We use durationSec as a "url" field for link type
    ...
    // Text message below link card (optional)
    if (typeof msg.durationSec === 'string' && msg.durationSec !== msg.text) {
      // url in durationSec handled above
    }

  } else if (msg.kind === 'viewonce') {
    // View-once media — 👁 icon, no preview
  ...
  // Meta (time + tick) — hidden for sticker
  if (msg.kind !== 'sticker') {
...
/**
 * Find a message by a "target" token:
 * - "#12" or "12" -> message id
 * - exact text match
 * - partial text match
 * - speaker name -> last message from that speaker
 */
